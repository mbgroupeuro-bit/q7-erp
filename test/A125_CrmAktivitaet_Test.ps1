# A125_CrmAktivitaet_Test.ps1
# Testet: create/findAllByKontakt/remove fuer CrmAktivitaet + Mandantentrennung
# Ausfuehren: cd "D:\Projekt2027\ERP System\test" ; .\A125_CrmAktivitaet_Test.ps1
...
# --- Testkontakt anlegen (Voraussetzung fuer Aktivitaet) ---
$createKontaktBody = @{ name = "Aktivitaet Test Kontakt" } | ConvertTo-Json
try {
...
# --- create() Aktivitaet ---
$createAktivitaetBody = @{
    crmKontaktId = $kontaktId
    text         = "Erstes Telefonat gefuehrt, Interesse an Produkt X."
} | ConvertTo-Json
...
try {
    ...
    Write-Result "create() - Aktivitaet angelegt" ($null -ne $aktivitaet.id)
...
# --- create() mit fremder/ungueltiger crmKontaktId soll fehlschlagen ---
$invalidKontaktBody = @{
    crmKontaktId = "00000000-0000-0000-0000-000000000000"
    text         = "Sollte fehlschlagen"
} | ConvertTo-Json
try {
    Invoke-RestMethod -Uri "$baseUrl/crm-aktivitaet" -Method Post -Headers $headersA -Body $invalidKontaktBody -ContentType "application/json" -ErrorAction Stop
...
try {
    ...
    $gefunden = $liste | Where-Object { $_.id -eq $aktivitaetId }
    Write-Result "findAllByKontakt() - Aktivitaet enthalten" ($null -ne $gefunden)
} catch {
...
# --- remove() von B auf A's Aktivitaet ---
try {
    Invoke-RestMethod -Uri "$baseUrl/crm-aktivitaet/$aktivitaetId" -Method Delete -Headers $headersB -ErrorAction Stop
    Write-Result "IDOR-Schutz remove() - B darf A's Aktivitaet nicht loeschen" $false
} catch {
    $statusCode = $_.Exception.Response.StatusCode.value__
    Write-Result "IDOR-Schutz remove() - B bekommt 404 (erwartet)" ($statusCode -eq 404)
}

# --- create() von B mit A's crmKontaktId (verknuepfte Abfrage IDOR, Szenario 3) ---
$crossTenantBody = @{
    crmKontaktId = $kontaktId
    text         = "B versucht Aktivitaet auf A's Kontakt anzulegen"
} | ConvertTo-Json
try {
    Invoke-RestMethod -Uri "$baseUrl/crm-aktivitaet" -Method Post -Headers $headersB -Body $crossTenantBody -ContentType "application/json" -ErrorAction Stop
